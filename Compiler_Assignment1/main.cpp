...
enum TokenType{
                ...
                ASSIGN, EQUAL, LESS_THAN ,BIGGER_THAN,
...
const char* TokenTypeStr[]=
            {
                "If", "Then", "Else", "End", "Repeat", "Until", "Read", "Write",
                "Assign", "Equal", "LessThan","BiggerThan",
                "Plus", "Minus", "Times", "Divide", "Power","And"
                "SemiColon",
...
// mathexpr -> ExprAbs { (+|-) ExprAbs } left associative
// ExprAbs -> term { & term } left associative
...
enum ExprDataType {VOID, INTEGER, BOOLEAN,DOUBLE,STRING};

// Used for debugging only /////////////////////////////////////////////////////////
const char* ExprDataTypeStr[]=
            {
                "Void", "Integer", "Boolean" , "Double" , "string"
            };
...
TreeNode* MathExpr(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    while(ppi->next_token.type==PLUS || ppi->next_token.type==MINUS)
    {
        ...
        new_tree->child[0]=tree;
        Match(pci, ppi, ppi->next_token.type);
        new_tree->child[1]=ExprAbs(pci, ppi);
...
int Evaluate(TreeNode* node, SymbolTable* symbol_table, int* variables)
{
    ...
    if(node->oper==AND) return andfun(a,b);
